...
# workflow on GitHub (the hosted OS × Python matrix, mypy, build, docker)
# instead of on the operator host, which producers and reviewers share and
# which lacks the three-OS Python matrix.
# Copied from relux-works/curator scripts/remote-gate.sh (header adapted);
